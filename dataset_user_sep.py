...
#     #print("data_loader:",len(labels), f"{time.time()-start_time:.6f}") --> 10 avrg sec
#     if not ARGS.get_user_ft:
#         input_tsr = torch.cat([torch.tensor(targets), torch.tensor(wins), torch.tensor(fails)],dim=1)
#     else : 
#         input_tsr = torch.cat([torch.tensor(users), torch.tensor(targets), torch.tensor(wins), torch.tensor(fails)],dim=1)
#     #https://github.com/pytorch/pytorch/issues/5039

#     return {
#         'label': torch.as_tensor(labels), #(batch, 1)
#         'static': input_tsr, #(batch, feat_size)
#         'dynamic': torch.as_tensor(questions), #(batch, seq_size)
#     }

def get_sequence_fm_a(batch):
    ...
    for data_path, num_of_interactions in zip(batch_data_path, batch_num_interacts):
        ...
        for term, line in enumerate(sliced_data):
            ...
            if answer:
                ...
                input_list.append(item + QUESTION_NUM[ARGS.dataset_name])
    ...
    #print("data_loader:",len(labels), f"{time.time()-start_time:.6f}") --> 10 avrg sec
    if not ARGS.get_user_ft:
        input_tsr = torch.cat([torch.tensor(target_ids), torch.tensor(wins), torch.tensor(fails)],dim=1)
    else : 
        input_tsr = torch.cat([torch.tensor(users), torch.tensor(target_ids), torch.tensor(wins), torch.tensor(fails)],dim=1)
    #https://github.com/pytorch/pytorch/issues/5039
    return {
        'label': torch.as_tensor(labels), #(batch, 1)
        'fm_input': input_tsr, #(batch, feat_size)
